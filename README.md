...
Показывает на светодиоде текущее состояние оркестратора и слушает кнопку
BOOT для локального mute микрофона.

Полное описание протокола — в [`protocol.md`](protocol.md).

### Как горит светодиод

Плата принимает от оркестратора имя состояния и локально рисует паттерн
через PWM. Все переходы моментальные — без плавных перетеканий между
паттернами.

| Состояние | Что видно на LED | Когда включается |
|---|---|---|
| `boot` | Плавное дыхание, 1 Гц, 0% ↔ 100% | Сразу после сброса платы |
| `idle` | Очень медленное дыхание, 10% ↔ 25% (период 5 с) | Ассистент ждёт wake word |
| `listening` | Ровно 100%, без мигания | Услышал wake word, пишет команду с микрофона |
| `thinking` | Быстрое дыхание, 2 Гц, 30% ↔ 100% | Идёт STT / память / генерация LLM |
| `speaking` | Резкий пульс 4 Гц, 40% ↔ 100% (квадратной волной) | Проигрывается TTS-ответ |
| `guest` | Двойной блик каждые 2 секунды | Ассистент не узнал говорящего |
| `error` | Ровное быстрое мигание ~8 Гц | Что-то упало в пайплайне |
| `mute` (локально) | Короткий blip (~60 мс, 60% яркости) раз в 2.5 с | Кнопкой включён mute |

Пока mute включён, паттерн muted перекрывает всё остальное — команды от
оркестратора запоминаются, но не отрисовываются. Как только mute снимут,
LED сразу вернётся к последнему запрошенному состоянию.

### Кнопка BOOT

Кнопка обрабатывается **на самой плате** (GPIO0, внутренний pull-up),
чтобы mute оставался рабочим визуально даже при обрыве связи с хостом.

| Действие | Что делает плата | Что шлёт в оркестратор |
|---|---|---|
| Короткое нажатие (≥ 30 мс, отпустил до 800 мс) | Переключает флаг mute локально, сразу рисует паттерн muted / возвращается к последнему состоянию | `EVT btn_short mute=on` или `EVT btn_short mute=off` |
| Долгое удержание (≥ 800 мс) | Ничего локально не меняет | `EVT btn_long` (сейчас зарезервировано, кандидаты — прерывание TTS или sync) |

Оркестратор на `EVT btn_short` сам взводит свой флаг `self.mute`: пока он
поднят, wake word не запускается и микрофон не открывается. То есть кнопка
физически глушит вход, а не просто гасит светодиод.
